...
export default function AddCardClient({ initialCollection }: any) {
  ...
  const handleAddCard = async () => {
    ...
    formData.append('category', initialCollection.category || '');
    imageFiles.forEach(file => formData.append('images', file));

    try {
      const result = await createCard(formData);
      if (result.success && result.cardId) {
        toast({ title: 'Card Added!', description: `"${title}" has been added to your collection.` });
      ...
      console.error(e);
      toast({ title: 'Error Adding Card', description: e.message || 'An unexpected error occurred.', variant: 'destructive' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="container py-8">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold font-headline">Add Card to "{initialCollection.name}"</h1>
          ...
          <CardContent className="p-6 grid gap-6">
            <div className="grid gap-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs text-muted-foreground">Upload Guideline: Prefer 3:2 aspect ratio.</span>
              </div>
              ...
              <Input ref={fileInputRef} type="file" className="hidden" accept="image/png, image/jpeg, image/webp" onChange={handleImageSelect} disabled={isSaving || imageFiles.length >= MAX_IMAGES} multiple />
            </div>
